    ...
    private static class Filters {
        ...
        public static final Predicate<Parser.Fork> repeatFilter = fork -> {
            if (!Context.parserParams.isRepeatFork()) {
                return !Context.forksCache.containsKey(new MathUtils.ForkKey(fork.betInfo1().BK_name().trim(), fork.betInfo1().BK_event_id().trim(), fork.betInfo1().BK_bet().trim()))
                    && !Context.forksCache.containsKey(new MathUtils.ForkKey(fork.betInfo2().BK_name().trim(), fork.betInfo2().BK_event_id().trim(), fork.betInfo2().BK_bet().trim()));
            }
            return true;
        };
